...
# ------------------------------------------------------------------------------
# -----------   ARTP_gene_simple   ---------------------------------------------
# data: rows contain variables, columns contain the original p-value followed
#       by p-values from bootstrapped data
#' @export
ARTP <- function(daten = data,
                 J = n_trunc,
                 n_perm = n_permutations)
{

  if(J > nrow(daten)) stop("J must NOT be greater than nrow(daten)")
